...
Every scheduled News Radar task instance must load the active skill, monitor contract, run contract, specialized-lanes contract, price-monitor live-source contract, sell-discipline contract, Nancy Pelosi congressional-disclosure tracker contract, AI Efficiency Watch contract, and AI Efficiency Watch state adapter before scanning. The active version applies beginning with the first scheduled occurrence after this activation commit.
...
Also load `news-radar-investing/references/ai-efficiency-watch-state.md` for the concrete Library fallback at `/News Radar/AI Efficiency Watch`, verified initialization, snapshot reconciliation, and activation-aware weekly timing. It controls the lane's fallback-state adapter and first eligible weekly boundary. The initial twelve-company baseline is PENDING, not completed evidence; the first weekly summary is September 25, 2026, at 15:00 Toronto time. Do not retroactively treat a pre-activation Friday as missed.
